...
export class CartService {
  ...
  //Data variable to store cart information on the server (here server is angular / frontend)
  ...
  // OBSERVABLES FOR THE COMPONENTS TO SUBSCRIBE (observable naming condition should end with $)
  ...
  constructor(private http:HttpClient,
    ...
    {
      //Local storage is not empty and has some information in it
   ...
   AddProductToCart(id:number,quantity?:number){
    this.productService.getSingleProduct(id).subscribe(prod=>{
      ...
      // 2. If the cart has some items
        ...
        let index = this.cartDataServer.data.findIndex(p=>p.product.id == prod.id); //-1 or a +ve value

        // a. if the item is already in the cart => index is +ve value
        if (index!==-1){
